...
parser.add_option("-n", "--periods", dest="periods", default=9, help="the number of periods to analyze during the duration of the tv_delay. Default=9")
parser.add_option("-o", "--output-file", dest="file", help="set the config file to execute. Default=spec_best.cfg", default=os.path.dirname(os.path.abspath(__file__)) + "\spec_best.cfg", metavar="<filename>")
...
"""
if ":" in options.ip:
    colonPos = options.ip.index(":")
    options.port = options.ip[colonPos + 1:]
    options.ip = options.ip[:colonPos]

"""

if ":" in options.ip:
    options.ip, options.port = options.ip.split(':')
